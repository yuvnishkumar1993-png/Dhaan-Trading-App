...
st.markdown("## ⚡ Institutional Quant Terminal Pro — Ultimate Terminal")
st.markdown("---")
...
# --- 5. QUANTITATIVE CALCULATION ENGINE (ACCURATE FORMULAS) ---
def calculate_advanced_metrics(df, spot, lot):
    ...
    for _, row in df.iterrows():
        ...
        # Correct GEX calculation formula: Gamma Exposure per 1% move
...
# --- 6. MATRIX LAYOUT (OPTION CHAIN TABLE) ---
if show_greeks:
    matrix_cols = [
...
# --- 7. ADVANCED GRAPHICAL TERMINAL (ALL 10 MODULES + SIGNALS) ---
st.markdown(f"### 🖥️ ADVANCED GRAPHICAL TERMINAL & SIGNALS (ALL 10 MODULES) — {selected_symbol}")
...
with t1:
    if HAS_PLOTLY:
        fig = go.Figure()
        fig.add_trace(go.Bar(x=chain_df['Strike'], y=chain_df['Raw_CE_OI'], name='CE OI (Res